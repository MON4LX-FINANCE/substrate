...
pub fn store<T: Config>(
	...
	storage_meter: &mut StorageMeter<T>,
) -> DispatchResult
where
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>,
{
	...
	// in order to avoid accessing an additional storage item.
	storage_meter.charge(&Diff {
		bytes_added: prefab_module.encoded_size().saturating_add(prefab_module.code.len()) as u32,
		items_added: 2,
		..Default::default()
	})?;
	...
	<CodeStorage<T>>::mutate(&code_hash, |existing| match existing {
		...
		None => {
			let orig_code = prefab_module.original_code.take().expect("TODO");
